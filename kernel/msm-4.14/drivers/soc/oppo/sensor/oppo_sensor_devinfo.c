...
static void oppo_sensor_parse_dts(struct platform_device *pdev)
{
	...
	rc = of_property_read_u32(node, "ps-parms-id", &value);
	if (rc) {
		chip->ps_parms_id = 0;//default params id
	} else {
		chip->ps_parms_id  = value;
	}
	rc = of_property_read_u32(node, "reg-num", &value);
	if (rc) {
		chip->reg_num= 0;//default params id
	} else {
		chip->reg_num= value;
	}
	...
	if(chip->reg_num >= 2)
	{
		size = of_property_count_elems_of_size(node, "reg-table", sizeof(uint32_t));
		pr_info("%s [size = %d] \n", __func__,size);
		if(size < 256){

			reg_table = kzalloc(sizeof(uint32_t)*size, GFP_KERNEL);
			if (!reg_table)
				return ;

			if (of_property_read_u32_array(node, "reg-table", reg_table, size) < 0) {
				pr_info("get reg table error");
			} else {
				for (i = 0 ;i < size; i++)
				{
					chip->reg_table[i] = reg_table[i];
					pr_info("%s [ %d] \n", __func__,chip->reg_table[i]);
				}
			}
			kfree(reg_table);
		}
...

{
	...
	*off += count;
    ...
    gpio_export(input,0);
	if((input != gdata->red_max_lux) && (input < 65535)){
...

{
	...
	*off += count;
	...
	if((input != gdata->white_max_lux) && (input < 65535)){
...

{
	...
	*off += count;
	...
	if((input != gdata->blue_max_lux) && (input < 65535)){
...

{
	...
	*off += count;
	...
	if((input != gdata->green_max_lux) && (input < 65535)){
...

{
	...
	*off += count;
	...
	if((input != gdata->cali_coe) && (input < 65535)){
...

{
	...
	*off += count;
	...
	if((input != gdata->row_coe) && (input < 65535)){
		gdata->row_coe= input;
	}
