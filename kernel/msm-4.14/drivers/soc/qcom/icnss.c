...
#ifdef VENDOR_EDIT
//Guotian.Wu@PSW.CN.WiFi.Basic.Log.1162003, 2020/02/28,
//Add for when wifi fw crash trigger panic capture full dump
#define OPPO_CAPTURE_WIFIFULLDUMP        1
#define OPPO_NOT_CAPTURE_WIFIFULLDUMP        0
bool need_wlan_fw_crash_panic = false;
#endif /* VENDOR_EDIT */
...
//Guotian.Wu@PSW.CN.WiFi.Basic.Log.1162003, 2020/02/28,
//Add for when wifi fw crash trigger panic capture full dump
static void icnss_create_wlan_crash_panic_kobj(void);
static ssize_t icnss_show_wlan_fw_crash(struct device_driver *driver, char *buf)
{
	bool wlan_fw_crash_panic = need_wlan_fw_crash_panic;
	return sprintf(buf, "%s", (wlan_fw_crash_panic ? "true" : "false"));
}

static ssize_t icnss_store_wlan_fw_crash(struct device_driver *driver, const char * buf, size_t count)
{
  int need_panic = 0;
  need_panic = buf[0] - '0';
  printk("test buf=%s,need_panic=%d,count=%d",buf,need_panic,count);
  switch(need_panic){

    case OPPO_CAPTURE_WIFIFULLDUMP:
      printk("capture wlan full dump");
      need_wlan_fw_crash_panic = true;
		  panic("wifi fw crash");
      break;
    case OPPO_NOT_CAPTURE_WIFIFULLDUMP:
      need_wlan_fw_crash_panic = false;
      printk("not capture wlan full dump");
  }
	return count;
}

struct driver_attribute wlan_fw_crash_panic_attr = {
	.attr = {
		.name = "wlan_fw_crash_panic",
		.mode = S_IRUGO|S_IWUGO,
	},
	.show = icnss_show_wlan_fw_crash,
  .store = icnss_store_wlan_fw_crash,
};
#endif /* VENDOR_EDIT */
...
static int icnss_probe(struct platform_device *pdev)
{
	...
	//Guotian.Wu@PSW.CN.WiFi.Basic.Log.1162003, 2020/02/28,
	//Add for when wifi fw crash trigger panic capture full dump
	icnss_create_wlan_crash_panic_kobj();
	#endif /* VENDOR_EDIT */
...
//Add for when wifi fw crash trigger panic capture full dump
static void icnss_create_wlan_crash_panic_kobj(void) {
	if (driver_create_file(&(icnss_driver.driver), &wlan_fw_crash_panic_attr)) {
		icnss_pr_info("failed to create %s", wlan_fw_crash_panic_attr.attr.name);
	}
}
#endif /* VENDOR_EDIT */
