...
#ifdef VENDOR_EDIT
/* LiPing-M@PSW.MM.Display.LCD.Stability,2019-10-12 Add for panel production date serial number */
	if ((bl_lvl > 1) && (readcount == 0) && (get_oppo_display_power_status() == OPPO_DISPLAY_POWER_ON)) {
...
extern int oppo_display_update_osc_clk(void);
extern int oppo_display_dynamic_clk_update_osc_clk(int clk_rate);
int dsi_display_set_power(struct drm_connector *connector,
		int power_mode, void *disp)
{
	struct dsi_display *display = disp;
