...
static bool oppo_frist_set_seed = false;
...
#define PANEL_OLED_MAX_BRIGHTNESS	1023
...
#ifdef VENDOR_EDIT
//Jiasong.Zhong@PSW.MM.Display.LCD.Stable, 2020/04/17, Add for dynamic OSC clock
...
int oppo_display_update_osc_clk(void)
{
	...
	if(oppo_display_vendor == OPPO_SAMSUNG_AMS643XF01_DISPALY_FHD_PLUS_CMD_PANEL){
	...
	if(oppo_display_vendor == OPPO_SAMSUNG_AMB655UV01_DISPALY_FHD_PLUS_90FPS_PANEL){
...
struct device_attribute *attr, char *buf) {
	...
	if(get_oppo_display_power_status() == OPPO_DISPLAY_POWER_ON) {
		...
		ret = dsi_display_read_panel_reg(get_main_display(),0xDA, read, 1);
		if(ret < 0) {
...
static ssize_t oppo_display_get_dimlayer_backlight(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d %d %d %d %d %d %d %d\n", oppo_dimlayer_bl_on_vblank, oppo_dimlayer_bl_off_vblank,oppo_dimlayer_bl_alpha,
...
static ssize_t oppo_display_set_dimlayer_backlight(struct device *dev,
                               ...
                               const char *buf, size_t count)
{
	sscanf(buf, "%d %d %d %d %d %d %d %d", &oppo_dimlayer_bl_on_vblank, &oppo_dimlayer_bl_off_vblank,&oppo_dimlayer_bl_alpha,
		&oppo_dimlayer_bl_alpha_value, &oppo_dimlayer_dither_threshold,
...
static uint64_t panel_max_brightness = 0;
static ssize_t oppo_display_get_max_brightness(struct device *dev,
		struct device_attribute *attr, char *buf) {
	panel_max_brightness = PANEL_OLED_MAX_BRIGHTNESS;
...
static DEVICE_ATTR(max_brightness, S_IRUGO|S_IWUSR, oppo_display_get_max_brightness, NULL);
static DEVICE_ATTR(mipi_clk_rate_hz, S_IRUGO|S_IWUSR, oppo_display_get_mipi_clk_rate_hz, NULL);
static DEVICE_ATTR(osc_clock, S_IRUGO|S_IWUSR, oppo_display_get_osc_clk, oppo_display_set_osc_clk);
