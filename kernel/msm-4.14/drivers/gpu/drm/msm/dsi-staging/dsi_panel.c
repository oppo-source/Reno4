...
#ifdef VENDOR_EDIT
//Jiasong.Zhong@PSW.MM.Display.LCD.Stable, 2020/03/25, Add for power off Sequence
				...
				struct dsi_parser_utils *utils)
{
	...
	if (rc) {
		pr_info("oppo,fod-on-vblank is not defined, rc=%d\n", rc);
		priv_info->fod_on_vblank = 0;
	} else {
		priv_info->fod_on_vblank = val;
		pr_info("oppo,fod-on-vblank is %d", val);
	}
	...
	if (rc) {
		...
		pr_info("oppo,fod-off-vblank is %d", val);
	}
