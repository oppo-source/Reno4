...
// Liujie.Xie@TECH.Kernel.Sched, 2020/04/30, add for improving animation issue
...
// Liujie.Xie@TECH.Kernel.Sched, 2019/09/20, add for ui first 2.0
	if (!fbt_env.fastpath && sysctl_uifirst_enabled && sysctl_launcher_boost_enabled &&
		p->static_ux == 2 && !test_ux_task_cpu(target_cpu)) {
