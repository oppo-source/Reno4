...
static ssize_t target_loads_show(struct gov_attr_set *attr_set, char *buf)
{
	...
	for (i = 0; i < tunables->ntarget_loads; i++)
		ret += snprintf(buf + ret, PAGE_SIZE, "%u%s", tunables->target_loads[i],
			i & 0x1 ? ":" : " ");
	snprintf(buf + ret - 1, PAGE_SIZE, "\n");
	spin_unlock_irqrestore(&tunables->target_loads_lock, flags);
	return ret;
}
...
#define __ATTR_RWRW(_name) __ATTR(_name, 0664, _name##_show, _name##_store)
//#endif /*COLOROS_EDIT*/

static struct governor_attr up_rate_limit_us = __ATTR_RW(up_rate_limit_us);
static struct governor_attr down_rate_limit_us = __ATTR_RW(down_rate_limit_us);
static struct governor_attr hispeed_load = __ATTR_RWRW(hispeed_load);
